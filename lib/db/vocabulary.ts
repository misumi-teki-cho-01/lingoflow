...
export async function getSavedVocabularyForTranscript(
  ...
  for (const row of data) {
    const vocab = Array.isArray(row.user_vocabulary) ? row.user_vocabulary[0] : row.user_vocabulary;
...
export async function saveVocabularyAndAnnotations(
  ...
  try {
    ...
    if (
      ...
      transcriptInfo.transcripts.length === 0
    ) {
    ...
    const vocabMap = new Map<
      string,
      { user_id: string; canonical_form: string; explanation: string }
    >();
    ...
    const { data: vocabResult, error: vocabErr } = await db
      ...
      .upsert(vocabInserts, {
        onConflict: 'user_id,canonical_form',
    ...
    const vocabIdByCanonicalForm = new Map(vocabResult.map((v) => [v.canonical_form, v.id]));
    ...
    if (sourceMode === 'cc' && ccSelections.length > 0) {
      const ccInserts = ccSelections.map((sel) => {
        ...
        return {
          ...
          vocabulary_id: def
            ? (vocabIdByCanonicalForm.get(def.canonical_form.trim()) ?? null)
            : null,
      ...
      for (const row of ccInserts) {
        ...
        if (deleteErr)
          console.error('[DB: Annotations] Failed to replace CC annotation:', deleteErr);
      }
    ...
    const { error: annoErr } = await db.from('user_annotations').insert(annotationInserts);
